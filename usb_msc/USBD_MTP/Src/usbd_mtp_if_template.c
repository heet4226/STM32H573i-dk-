...
static uint32_t  USBD_MTP_Itf_GetObjParentObject(uint32_t Param);
...
USBD_MTP_ItfTypeDef USBD_MTP_fops =
{
  ...
  USBD_MTP_Itf_GetIdx,
  USBD_MTP_Itf_GetObjParentObject,
...
static uint8_t USBD_MTP_Itf_Init(void)
{
    /* Ensure file exists */
    FX_FILE file;
    char txt[] = {"hello from heet"};
    fx_file_create(&sram_disk, "heet.txt");
    fx_file_open(&sram_disk, &file, "heet.txt", FX_OPEN_FOR_WRITE);
    fx_file_write(&file, txt, sizeof(txt)-1);
    fx_file_close(&file);
    return USBD_OK;
}
...
//
//  return obj_len;
	return strlen("heet.txt"); // "heet.txt"
}
...
//  UNUSED(obj_len);
//  UNUSED(buf);
	 const char name[] = "heet.txt";
	    for (int i = 0; i < obj_len; i++)
	        buf[i] = (uint16_t)name[i];   // convert ASCII → UTF-16
}
...
static uint32_t USBD_MTP_Itf_GetObjectSize(uint32_t Param)
{
	 ...
	 char txt[] = {"hello from heet"};
	    ULONG actual_size = 0;
	    fx_file_open(&sram_disk, &file, "heet.txt", FX_OPEN_FOR_READ);
	    fx_file_seek(&file, 0);
	    fx_file_read(&file, NULL, 0, &actual_size);
	    fx_file_close(&file);
	    return strlen(txt);
}
...
//  uint64_t max_cap = 0U;
	...
	return (uint64_t)FX_SRAM_DISK_SIZE;
}
...
//  uint64_t f_space_inbytes = 0U;
	  ...
	  return (uint64_t)(FX_SRAM_DISK_SIZE / 2);
}
...
{
	 FX_FILE file;
	    if (fx_file_open(&sram_disk, &file, "heet.txt", FX_OPEN_FOR_WRITE) == FX_SUCCESS)
	    {
	        fx_file_write(&file, buff, len);
	        fx_file_close(&file);
	    }
}
...
static uint32_t USBD_MTP_Itf_GetContainerLength(uint32_t Param1)
{
    ...
    if (fx_file_open(&sram_disk, &file, "heet.txt", FX_OPEN_FOR_READ) == FX_SUCCESS)
    {
        size = file.fx_file_current_file_size;
        fx_file_close(&file);
    }

    // Add 12 bytes for the MTP container header
    return size + 12;
}
...
static uint32_t USBD_MTP_Itf_ReadData(uint32_t Param1, uint8_t *buff, MTP_DataLengthTypeDef *data_length)
{
	FX_FILE file;
	    ...
	    /* Open file */
	    status = fx_file_open(&sram_disk, &file, "heet.txt", FX_OPEN_FOR_READ);
	    if (status != FX_SUCCESS)
	        return 1;

	    /* Always start at beginning for first packet */
	    if (data_length->readbytes == 0)
	        fx_file_seek(&file, 0);

	    /* Read exactly 512 or remaining bytes */
	    ULONG remaining = file.fx_file_current_file_size - data_length->readbytes;
	    ULONG to_read = (remaining > 512) ? 512 : remaining;

	    status = fx_file_read(&file, buff, to_read, &actual);
	    fx_file_close(&file);

	    data_length->temp_length = actual;
	    data_length->readbytes  += actual;
	    data_length->totallen    = file.fx_file_current_file_size;

	    /* Pad remainder of buffer with zeros if partial read */
	    if (actual < 512)
	        memset(buff + actual, 0, 512 - actual);
