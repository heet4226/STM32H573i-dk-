...
{
  (uint8_t *)"DFU MEDIA",
//(uint8_t *) "@Internal Flash  /0x08000000/04*016Ka,01*064Kg,07*128Kg",
  MEM_If_Init,
  MEM_If_DeInit,
  MEM_If_Erase,
  MEM_If_Write,
...
uint16_t MEM_If_Erase(uint32_t Add)
{
	 FLASH_EraseInitTypeDef EraseInitStruct;
	    ...
	    EraseInitStruct.Sector = (Add - FLASH_BASE) / FLASH_SECTOR_SIZE;
...
uint16_t MEM_If_Write(uint8_t *src, uint8_t *dest, uint32_t Len)
{
	HAL_FLASH_Unlock();
	    ...
	    for (uint32_t i = 0; i < Len; i += 16)   // 128-bit programming
	    {
