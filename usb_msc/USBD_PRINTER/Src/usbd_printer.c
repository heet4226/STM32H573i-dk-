...
__ALIGN_BEGIN static uint8_t USBD_PRNT_CfgDesc[] __ALIGN_END =
{
  ...
  0x01,                                /* bInterfaceSubClass: Printer Subclass */
...
//  (void)USBD_LL_PrepareReceive(pdev, PRNTOutEpAdd, hPRNT->RxBuffer, mps);
  static uint8_t PRNT_RxBuffer[64];
  hPRNT->RxBuffer = PRNT_RxBuffer;

  /* Init  physical Interface components */
  ((USBD_PRNT_ItfTypeDef *)pdev->pUserData[pdev->classId])->Init();

  /* Prepare Out endpoint to receive next packet */
  (void)USBD_LL_PrepareReceive(pdev, PRNTOutEpAdd, hPRNT->RxBuffer, sizeof(PRNT_RxBuffer));
